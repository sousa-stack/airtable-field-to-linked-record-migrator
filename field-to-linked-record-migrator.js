 ...
 */
function renderProgress(headerText, progressLine) {
    output.clear();
    output.markdown(headerText + '\n\n' + progressLine);
}
...
async function runMigration() {
    const errors = [];
    let caseNormalizedCount = 0;

    // ==========================================================
    // SETUP — Interactive Field Selection
    // ==========================================================
    ...
    const linkedRecordField = await input.fieldAsync('Pick the linked record field to populate:', sourceTable);

    // ==========================================================
    // CONFIRMATION — Show migration plan
    // ==========================================================
    ...
    for (const rec of sourceRecords) {
        if (getCellString(rec, sourceField) !== '') {
            recordsToUpdate++;
        }
    }
    ...
    const confirmStart = await input.buttonsAsync('Ready to proceed?', [
    ...
    // Extract unique values (preserve original casing for display)
    const uniqueValues = new Map(); // lowercase → original casing
    for (const rec of sourceRecords) {
        const val = getCellString(rec, sourceField);
        if (val !== '') {
            const key = val.toLowerCase();
            if (!uniqueValues.has(key)) {
                uniqueValues.set(key, val);
            }
        }
    }
    ...
    // Load target records and build case-insensitive lookup map
    const targetQuery = await targetTable.selectRecordsAsync({ fields: [targetMatchField] });
    const targetRecords = targetQuery.records;

    const targetLookup = {};       // lowercase value → record ID
    const targetOriginal = {};     // lowercase value → original casing in target
    for (const rec of targetRecords) {
        const val = getCellString(rec, targetMatchField);
        if (val !== '') {
            const key = val.toLowerCase();
            targetLookup[key] = rec.id;
            targetOriginal[key] = val;
        }
    }

    // Classify each unique value
    const willLink = [];   // { value, recordId, caseMatch }
    const willCreate = []; // { value }

    for (const [key, originalVal] of uniqueValues) {
        if (targetLookup[key]) {
            const targetVal = targetOriginal[key];
            const exactMatch = originalVal === targetVal;
            if (!exactMatch) caseNormalizedCount++;
            willLink.push({ value: originalVal, recordId: targetLookup[key], caseMatch: exactMatch });
        } else {
            willCreate.push({ value: originalVal });
        }
    }
    ...
    for (const item of willLink) {
        allPreviewItems.push({
            ...
            status: item.caseMatch ? '✅ exact match' : '⚠️ case-normalized',
        });
    }
    ...
    const previewHeaders = ['Value', 'Action', 'Status'];
    ...
    if (caseNormalizedCount > 0) {
        previewText += '⚠️ **' + caseNormalizedCount + '** value(s) matched via case-insensitive comparison (e.g. "Acme Corp" → "acme corp")\n\n';
    }

    previewText += mdTable(previewHeaders, previewRows) + '\n\n';
    ...
    const confirmPhase2 = await input.buttonsAsync('Confirm migration actions?', [
        ...
        for (const batch of createBatches) {
            ...
            try {
                ...
                for (let j = 0; j < batch.length; j++) {
                    targetLookup[batch[j].value.toLowerCase()] = newIds[j];
                    newRecordsCreated++;
                }
            } catch (e) {
    ...
    for (let ri = 0; ri < freshSourceRecords.length; ri++) {
        ...
        const targetId = targetLookup[val.toLowerCase()];
        if (!targetId) {
            errors.push({
                phase: 'Link',
                value: val,
    ...
    const summaryRows = [
        ...
        ['Case-normalized matches', String(caseNormalizedCount)],
    ...
    if (caseNormalizedCount > 0) {
        summaryReport += '⚠️ **' + caseNormalizedCount + '** match(es) were case-insensitive — values matched despite different capitalization.\n\n';
    }
